...
func main() {
	// Open the Excel file
	f, err := excelize.OpenFile("SOFAAMY.xlsx")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	for _, sheet := range f.GetSheetList() {
		...
		if sheet == "Sheet1" {
			csvFileName = "credits.csv"
		} else if sheet == "Sheet2" {
			csvFileName = "debits.csv"
		} else {
			continue // Skip sheets other than the first two
		}
		...
		fmt.Printf("Data extraction and conversion to %s completed successfully.\n", csvFileName)
	}
}
